...
def reward_function(lidar_data : list, current_pos, goal_pos, action, steering_angle) -> float:
    reward = 0.0
...
for episode in tqdm(range(EPISODES), desc="Training progress", unit="episodes"):
    ...
    while not done:
        ...
        reward = reward_function(current_state, gps_data, goal_position, action, steering_angle)
        total_reward += reward
